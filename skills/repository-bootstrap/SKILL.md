...
description: Initialize a repository created from the Skillforge template exactly once, establish required GitHub labels and project-specific top-level instructions, then remove this bootstrap skill and all of its template-only references.
---
...
Its successful terminal state is that:
...
Explicit invocation of this skill grants authority to perform the bootstrap directly on the repository's default branch without creating an issue or pull request.
...
Do not report bootstrap complete until these conditions are observed.
